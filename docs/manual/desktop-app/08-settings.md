...
If you work headless or want the configuration in version control, write the same three axes as a git-tracked sidecar file named `<SolutionName>.aicb.json` next to the `.sln` file. It carries the rules embedded (no database ID references), so it is portable, and both the desktop app and the headless MCP tools read it. Of the related MCP tools, `solution_config_status` only reads status and `init_solution_config` only gathers proposal material; `apply_solution_config` is the operation that writes the configuration database and sidecar.
